#!/bin/bash
# Qwen2.5-72B SFT on multi-node MUSA GPUs (Megatron-Core + TransformerEngine FP8).
#
# HF reference: Qwen/Qwen2.5-72B
#   hidden_size=8192, num_layers=80, num_heads=64, num_kv_heads=8,
#   head_dim=128 (requires --kv-channels 128), intermediate_size=29568,
...
# Qwen2.5-72B: 80 layers, hidden=8192, 64 heads, 8 KV heads, head_dim=128.
# hidden_size/num_heads = 128, so --kv-channels 128 is required.
MODEL_ARGS=(
    --num-layers 80
    --hidden-size 8192
...
if [ "${MOCK_DATA:-0}" = "1" ]; then
    DATA_ARGS="--mock-data --split 1 --tokenizer-type NullTokenizer --vocab-size 152064"
else
    DATA_ARGS="
        --data-path $DATA_PATH \
